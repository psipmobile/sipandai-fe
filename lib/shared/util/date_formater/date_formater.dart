...
formatTime(String? date) {
  ...
  DateTime dateConverted = DateTime.parse(date);
  return "${dateConverted.hour.toString().padLeft(2, '0')}:${dateConverted.minute.toString().padLeft(2, '0')}:${dateConverted.second.toString().padLeft(2, '0')} WIB";
}
...
String formatDateForView(String? originalDateString) {
  try {
    ...
    DateTime dateTime = originalFormat.parse(trimString(originalDateString));
